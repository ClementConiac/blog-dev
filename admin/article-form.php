...
if (isset($_GET['article_id']) && isset($_GET['action']) && $_GET['action'] == 'delete_image') {
    $queryDeleteImage = $db->prepare('SELECT * FROM image WHERE article_id = ?');
    $queryDeleteImage->execute(array($_GET['article_id']));
    $imageDelete = $queryDeleteImage->fetch();
    /*var_dump($imageDelete);*/
    if ($imageDelete) {
        unlink('../img/article/' . $imageDelete['name']);
    }
                ...
                <?php if(isset($article)) : ?>
                    <div class="tab-pane container-fluid <?php if(isset($_POST['add_image'])): ?>active<?php endif; ?>" id="images" role="tabpanel">
                        ...
                        <div class="row flex-wrap">
                            <?php
                            $queryListImages = $db->prepare('SELECT * FROM image WHERE article_id = ?');
                            $queryListImages->execute(array($_GET['article_id']));
                            foreach($queryListImages as $imageArticle) : ?>

                                <div class="gal-img">
                                    <img class="mr-2" src="../img/article/<?php echo $imageArticle['name']; ?>" alt="<?php echo $imageArticle['caption'];?>">
                                    <div class="btn-supp-del">
                                        <a onclick="return confirm('Are you sure?')" href="article-form.php?article_id=<?php echo $article['id']; ?>&action=delete_image" class="btn btn-danger mt-2">Supprimer</a>
                                        <a href="article-form.php?article_id=<?php echo $article['id']; ?>&action=edit" class="btn btn-warning mt-2">Modifier</a>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
